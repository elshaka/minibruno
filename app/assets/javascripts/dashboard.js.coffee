...
window.update_stats = (data) ->
  update_div = (temp, div_id, control_div_id) ->
    if temp?
      ...
      if temp.set_point?
        $(control_div_id).html("SP #{temp.set_point} °F <span class='label label-#{if temp.auto then 'success' else 'danger'} auto-label'>#{if temp.auto then 'Auto' else 'Manual'}</span>")
    else
      $(div_id).html("-- °F")
      $(control_div_id).html("--")
